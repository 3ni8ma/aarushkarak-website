...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      particles.forEach(p => {
        ...
        grad.addColorStop(0, 'rgba(var(--color-primary), 0.25)')
        grad.addColorStop(1, 'rgba(var(--color-primary), 0)')
        ctx.fillStyle = grad
        ctx.beginPath(); ctx.arc(p.x, p.y, p.r * 4, 0, Math.PI * 2); ctx.fill()
      })
      ctx.strokeStyle = 'rgba(var(--color-secondary), 0.06)'
      ctx.lineWidth = 0.5
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden" aria-label="Hero introduction">
      ...
      <div className="absolute top-1/4 -left-32 w-[500px] h-[500px] rounded-full bg-primary/20 blur-[120px] animate-orb pointer-events-none" aria-hidden="true" />
      <div className="absolute bottom-1/4 -right-32 w-[400px] h-[400px] rounded-full bg-secondary/20 blur-[100px] animate-orb pointer-events-none" style={{ animationDelay: '-3s' }} aria-hidden="true" />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full bg-accent/10 blur-[140px] animate-orb pointer-events-none" style={{ animationDelay: '-5s' }} aria-hidden="true" />
