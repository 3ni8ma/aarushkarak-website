    ...
    <ScrollReveal delay={index * 0.1}>
      ...
      >
        <div className="flex items-start justify-between mb-4" style={{ transform: 'translateZ(20px)' }}>
          ...
          <ExternalLink size={16} className="text-muted group-hover:text-primary transition-colors mt-1 shrink-0" />
        </div>
        ...
        <div className="flex flex-wrap gap-2" style={{ transform: 'translateZ(10px)' }}>
          {p.tags.map(t => (
            <span key={t} className="text-xs px-3 py-1 rounded-full" style={{ backgroundColor: tc.bg, color: tc.text }}>
              {t}
            </span>
          ))}
        </div>
