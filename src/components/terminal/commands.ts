...
export const commands: Record<string, CommandHandler> = {
  ...
  ls: ({ args }) => {
    ...
    return {
      text: dirs[target] || `ls: cannot access '${target}': No such directory`,
      type: "output",
    };
  },
  ...
  cd: ({ args }) => {
    ...
    if (pageMap[target]) {
      return {
        text: `cd: ${target}/ — navigating...`,
        navigate: pageMap[target],
      };
    }
    if (target === ".." || target === "~") {
  ...
  theme: ({ args, theme, themes, setTheme }) => {
    if (args.length === 0) {
      return {
        text: `Current theme: ${theme.label}\nAvailable: ${themes.map((t) => t.label).join(", ")}\nUsage: theme <name>`,
        type: "output",
      };
    }
    const name = args.join(" ").toLowerCase();
    const found = themes.find(
      (t) => t.label.toLowerCase() === name || t.id === name,
    );
    if (found) {
      setTheme(found.id);
    ...
    return {
      text: `Theme '${name}' not found. Available: ${themes.map((t) => t.label).join(", ")}`,
      type: "error",
    };
  },
  ...
  sudo: ({ args }) => {
    ...
    return {
      text: `Permission denied. ${args.join(" ")} requires elevated privileges.`,
      type: "error",
    };
  },
